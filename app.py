...
# Load dataset to recreate preprocessing steps
@st.cache_data
def load_and_preprocess_data():
    ...
    # One-hot encode 'Gender', 'OverTime', and 'Attrition'
...
# Load processed data and get columns
df_processed, scaler = load_and_preprocess_data()
X = df_processed.drop('Attrition_Yes', axis=1)
y = df_processed['Attrition_Yes']
feature_columns = X.columns

# Load model (you can train it inside the app or use joblib to load a saved model)
@st.cache_resource
def train_model():
    X_train, y_train = X, y
...
st.write("Enter employee information below to predict the likelihood of attrition.")
...
# Manual one-hot encoding for other categorical features
categorical_map = {
...
for col in categorical_map:
    encoded_input[col] = 1 if categorical_map[col] else 0
...
encoded_input[numerical_cols := scaler.feature_names_in_] = scaler.transform(encoded_input[numerical_cols])
