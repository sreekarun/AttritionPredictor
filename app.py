...
# --- Define model input features exactly as the model expects ---
model_features = [
...
# --- Build complete input dict with EXACT feature names ---
input_data = {
    # Numerical features
    'Age': age,
    ...
    # Categorical features (must match model_features exactly)
...
# Ensure all model features are included, set missing ones to 0
...
if model and st.button("Predict Attrition Risk"):
    try:
        ...
        # Debug: Show the features being sent to the model
        st.write("Features being sent to model:", input_df.iloc[0])
